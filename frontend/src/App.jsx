            ...
            {token && (
              <div className="hidden md:flex items-center gap-6 text-[10px] items-center font-bold tracking-widest text-slate-500">
                <span 
                  ...
                  className={cn("hover:text-white cursor-pointer transition-colors px-2 py-1", activeTab === 'dashboard' && "text-brand-cyan border-b-2 border-brand-cyan")}
                >DASHBOARD</span>
                <span 
                  onClick={() => setActiveTab('scans')}
                  className={cn("hover:text-white cursor-pointer transition-colors px-2 py-1", activeTab === 'scans' && "text-brand-cyan border-b-2 border-brand-cyan")}
                >SCANS</span>
                <span 
                  onClick={() => setActiveTab('reports')}
                  className={cn("hover:text-white cursor-pointer transition-colors px-2 py-1", activeTab === 'reports' && "text-brand-cyan border-b-2 border-brand-cyan")}
                >REPORTS</span>
              </div>
            )}
          </div>
            ...
            {user && (
              <div className="flex items-center gap-4 ml-4 pl-4 border-l border-white/10">
                <div className="flex flex-col items-end">
                  <span className="text-[10px] font-bold text-white uppercase">{user.full_name}</span>
                  <span className="text-[8px] font-medium text-slate-500 uppercase tracking-widest leading-none">Agent Active</span>
                </div>
                <button 
      ...
      {!token ? (
        ...
        {activeTab === 'dashboard' && (
          <div className="animate-in fade-in duration-500">
            ...
            {/* Dashboard Grid (Shown when no active scan) */}
            {!scanning && (
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-20 animate-in fade-in slide-in-from-bottom-8 duration-700">
                <div className="card tech-border">
                  ...
                  <div className="text-3xl font-black text-brand-cyan">{stats?.total_scans_analyzed?.toString().padStart(3, '0') || '000'}</div>
        ...
        {activeTab === 'scans' && (
          <div className="animate-in fade-in duration-500">
            <div className="flex items-center gap-4 mb-12 border-b border-white/10 pb-6">
              ...
              <h3 className="text-3xl font-black tracking-tighter uppercase">Scan History</h3>
            </div>
            <div className="grid grid-cols-1 gap-4">
              {history.map((scan) => (
                <div 
                  key={scan.id} 
                  onClick={() => { setResult(scan); setActiveTab('reports'); }}
                  className="group flex items-center justify-between p-6 bg-white/5 border border-white/5 hover:border-brand-cyan/30 cursor-pointer transition-all hover:bg-white/[0.08]"
                >
                  <div className="flex items-center gap-6">
                    <Globe className="w-8 h-8 text-slate-700 bg-black flex items-center justify-center rounded-sm" />
                    <div>
                      <div className="text-xl font-black tracking-tighter uppercase group-hover:text-brand-cyan transition-colors">
                        {new URL(scan.url).hostname}
                      </div>
                      <div className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">
                        Detected: {scan.vulnerabilities?.length || 0} Vulnerabilities
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-12">
                    <div className="text-right">
                      <div className="text-[10px] text-slate-500 font-bold uppercase mb-1">Severity</div>
                      <div className={cn(
                        "text-2xl font-black",
                        scan.predicted_severity_label === 'Critical' ? "text-red-600" :
                        scan.predicted_severity_label === 'High' ? "text-red-500" :
                        scan.predicted_severity_label === 'Medium' ? "text-yellow-500" : "text-brand-cyan"
                      )}>
                        {scan.ml_severity_score}
                      </div>
                    </div>
                    <ChevronRight className="w-6 h-6 text-slate-800 group-hover:text-brand-cyan transition-transform group-hover:translate-x-1" />
                  </div>
                </div>
              ))}
              {history.length === 0 && (
                <div className="text-center py-20 border border-dashed border-white/5 rounded-2xl">
                  <p className="text-slate-500 font-bold uppercase text-xs">No scan history available.</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'reports' && (
          <>
            {result ? (
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-white/10 pb-8">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-bold text-brand-cyan">ANALYSIS REPORT</span>
                    </div>
              ...
              <div className="text-center py-40 border border-dashed border-white/10 rounded-2xl animate-in fade-in duration-500">
                <ShieldAlert className="w-16 h-16 text-slate-700 mx-auto mb-4 opacity-20" />
                <h3 className="text-xl font-black text-slate-500 uppercase">No Active Report</h3>
                <p className="text-xs text-slate-600 mt-2">Initiate a scan or select one from history to view details.</p>
                <button 
                  onClick={() => setActiveTab('dashboard')}
                  className="mt-8 px-6 py-2 border border-brand-cyan/20 text-brand-cyan text-[10px] font-bold hover:bg-brand-cyan hover:text-black transition-all"
                >
                  BACK TO SCANNER
                </button>
              </div>
